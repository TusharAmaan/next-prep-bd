...
export default function ExamViewPage() {
  ...
  useEffect(() => {
    const fetchExam = async () => {
        try {
            // 1. Get Exam ID safely
            const examId = params?.id;
            
            // If ID is missing or invalid, stop loading and show error
            if (!examId || examId === 'undefined') {
                setError("Invalid Exam ID provided in URL.");
                setLoading(false);
                return;
            }
            ...
            console.log("Fetching Exam:", examId); // Debug Log

            // 3. Fetch Data (The "Blog Post" Content)
            ...
            if (dbError) {
                ...
                // RLS Policy blocks access -> throws error
                setError("Exam not found or access denied.");
            } else if (!data) {
                setError("Exam document is empty.");
            } else {
                setExam(data); // Success!
            }
  ...
  if (loading) return (
      <div className="h-screen flex flex-col items-center justify-center gap-3 bg-slate-50">
          ...
          <p className="text-sm font-bold text-slate-500 animate-pulse">Retrieving Exam Paper...</p>
      </div>
  );
  ...
  if (error || !exam) return (
      <div className="h-screen flex flex-col items-center justify-center gap-6 bg-slate-50 p-4">
          ...
          <div className="text-center">
              <h1 className="text-2xl font-black text-slate-800 mb-2">Unable to Load Exam</h1>
              <p className="text-slate-500 max-w-md mx-auto mb-6 leading-relaxed">
                  {error || "This content does not exist or you do not have permission to view it."}
              </p>
