...
export default function ExamViewPage() {
  ...
  // Safely extract ID (handle potential array or undefined)
  const rawId = params?.id;
  const examId = Array.isArray(rawId) ? rawId[0] : rawId;

  useEffect(() => {
    const fetchExam = async () => {
        // Reset state on every new attempt
        setError(""); 
        setLoading(true);

        // 1. Wait for ID to be ready
        if (!examId || examId === "undefined") {
            // Don't error yet, just wait. Next.js might be hydrating.
            return; 
        }
        ...
        console.log("Fetching Exam ID:", examId, "for User:", user.id);

        // 3. Fetch Exam (RLS enforces security)
        ...
        if (error) {
            ...
            setError("Exam not found or you do not have permission.");
        } else {
            setExam(data);
        }
        setLoading(false);
  ...
  if (loading) return (
      <div className="h-screen flex flex-col items-center justify-center gap-3">
          ...
          <p className="text-sm font-medium text-slate-500">Loading Paper...</p>
      </div>
  );
  ...
  if (error || !exam) return (
      <div className="h-screen flex flex-col items-center justify-center gap-6 bg-slate-50 p-4">
          <div className="bg-red-50 p-5 rounded-full shadow-sm"><ShieldAlert className="w-12 h-12 text-red-500"/></div>
          <div className="text-center">
              <h1 className="text-2xl font-black text-slate-800 mb-2">Access Denied</h1>
              <p className="text-slate-500 max-w-md mx-auto mb-6">{error || "This exam does not exist or belongs to another user."}</p>
              
              <Link href="/tutor/dashboard/my-exams" className="px-8 py-3 bg-indigo-600 text-white rounded-xl font-bold hover:bg-indigo-700 transition-all shadow-lg hover:shadow-indigo-200">
                  Return to My Exams
              </Link>
          </div>
      </div>
  ...
  return (
    <div className="min-h-screen bg-slate-100 flex flex-col font-sans text-slate-900">
       ...
       <div className="bg-white border-b border-slate-200 px-6 py-4 flex justify-between items-center shadow-sm sticky top-0 z-50 print:hidden">
           ...
           <button onClick={() => handlePrint()} className="flex items-center gap-2 bg-slate-900 text-white px-6 py-2.5 rounded-xl font-bold text-sm hover:bg-black shadow-lg transition-all">
