...
export default function ExamViewPage() {
  ...
  useEffect(() => {
    const fetchExam = async () => {
        ...
        if (!user) {
            // If not logged in, they can't view it (Security)
            router.push('/login');
            return;
        }

        // 2. Fetch Exam (RLS will automatically block if user_id doesn't match)
        ...
        if (error || !data) {
            ...
            setError("Exam not found or you do not have permission to view it.");
        } else {
            setExam(data);
        }
        setLoading(false);
  ...
  if (error || !exam) return (
      <div className="h-screen flex flex-col items-center justify-center gap-4 bg-slate-50">
          ...
          <Link href="/tutor/dashboard" className="px-6 py-2 bg-white border border-slate-200 rounded-lg text-sm font-bold shadow-sm hover:bg-slate-50">
              Return to Dashboard
          </Link>
      </div>
  );
  ...
  return (
    <div className="min-h-screen bg-slate-100 flex flex-col font-sans text-slate-900">
       ...
       {/* PAPER PREVIEW (This part gets printed) */}
       <div className="flex-1 overflow-y-auto p-8 flex justify-center print:p-0 print:overflow-visible">
           <div ref={printRef} className="bg-white shadow-2xl min-h-[297mm] w-[210mm] p-[20mm] relative print:shadow-none print:w-full print:h-auto print:p-0 print:m-0">
               ...
               <div className="text-center border-b-2 border-black pb-6 mb-8">
                   {/* Displays YOUR saved institute name */}
               ...
               <div className="mt-16 pt-6 border-t-2 border-slate-900 text-center break-before-auto">
                   ...
                   {/* Show watermark only if user was NOT pro when saving */}
