...
import { createClient } from "@/utils/supabase/client"; // FIX: Correct Client Import
import Link from "next/link";
import { 
  Search, Filter, Eye, Clock, Edit3, 
  MoreVertical, AlertCircle, CheckCircle2, Clock3 
} from "lucide-react";
import RejectionModal from "@/components/tutor/RejectionModal"; // We will create this below
...
export default function MyContentPage() {
  ...
  const supabase = createClient(); // FIX: Initialize client
  ...
  const fetchContent = async () => {
    ...
    let query = supabase
      .from('resources')
      .select('id, title, type, status, created_at, admin_feedback, subjects(title)')
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto py-8 px-6">
      ...
      <div className="flex flex-col md:flex-row justify-between items-end md:items-center gap-4">
        ...
        <Link href="/tutor/dashboard/create" className="bg-indigo-600 hover:bg-indigo-700 text-white px-5 py-2.5 rounded-xl font-bold text-sm shadow-lg shadow-indigo-200 transition-all flex items-center gap-2">
           + Upload New Content
        </Link>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
        <table className="w-full text-left border-collapse">
          ...
          <tbody className="divide-y divide-slate-100">
            {loading ? (
               <tr><td colSpan={4} className="p-12 text-center text-slate-400 font-medium">Loading content...</td></tr>
            ) : resources.length === 0 ? (
               <tr><td colSpan={4} className="p-12 text-center text-slate-400 font-medium">No content found in this category.</td></tr>
            ) : (
              resources.map((item) => (
                <tr key={item.id} className="hover:bg-slate-50/80 group transition-colors">
                  ...
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-4 text-xs font-bold text-slate-600">
                      <div className="flex items-center gap-1" title="Total Views">
                        ...
                        {/* Note: 'views' column needs to exist in DB, default to 0 if null */}
                        {item.views || 0}
                      </div>
                  ...
                  <td className="px-6 py-4">
                    ...
                    {item.status === 'rejected' && (
                      <button 
                        onClick={() => setSelectedRejection(item.admin_feedback || "No specific reason provided.")}
                  ...
                  <td className="px-6 py-4 text-right">
                    {/* FIX: Corrected URL to match your file structure */}
                    <Link href={`/tutor/dashboard/content/${item.id}`} className="inline-flex items-center gap-1 text-slate-400 hover:text-indigo-600 transition-colors font-bold text-xs">
