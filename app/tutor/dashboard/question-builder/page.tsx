...
import { 
  ...
  ChevronDown, RefreshCw, XCircle, Eye, X, BookOpen, MonitorPlay, EyeOff
} from "lucide-react";
...
export default function QuestionBuilder() {
  ...
  const [showPaperOverview, setShowPaperOverview] = useState(false); // NEW: Full Paper Preview
  ...
  useEffect(() => {
    const init = async () => {
      ...
      if (user) {
        ...
        if (prof) {
            // Logic: Is Pro? (Trial counts as Pro features)
  ...
  const handleSave = async () => {
    ...
    // COUNTING LOGIC:
    // By inserting into 'exam_papers', we are officially creating a "Saved Exam".
    // We can count the user's total exams by simply counting rows in this table later.
    ...
    <div className="flex flex-col h-[calc(100vh-64px)] bg-slate-100 font-sans text-slate-900">
      ...
      <div className="bg-white border-b border-slate-200 px-4 md:px-6 py-3 flex justify-between items-center shadow-sm z-30">
          ...
          <div className="flex gap-3">
              {/* NEW: View Question (Overview) Button */}
              <button 
          ...
          <div className="flex-1 overflow-y-auto bg-slate-200 p-4 md:p-8 flex justify-center">
              ...
              <div className="bg-white shadow-2xl w-[210mm] min-h-[297mm] p-[20mm] relative">
                  ...
                  <div className="mb-8">
                      ...
                      {showInstructions && (
                          <div className="text-sm text-slate-800 leading-relaxed p-2 rounded border border-transparent hover:border-slate-100 transition-colors">
                              <Editor 
                                  apiKey="koqq37jhe68hq8n77emqg0hbl97ivgtwz2fvvvnvtwapuur1"
                                  value={instructions} 
                                  onEditorChange={c => setInstructions(c)}
                                  init={{ height: 80, menubar: false, toolbar: false, statusbar: false }}
                              />
                          </div>
                      )}
                  </div>
                      ...
                      {selectedQuestions.length === 0 ? (
                          ...
                          selectedQuestions.map((q, idx) => (
                              <div key={q.id} className="relative group pl-2 hover:bg-slate-50 transition-colors rounded -ml-2 p-1 break-inside-avoid">
                                  <div className="flex gap-3 items-start">
                                      ...
                                      <div className="w-12 text-right">
                                          {isPro ? (
                                              <input 
                                                  type="number" 
                                                  className="w-8 text-right font-bold text-sm bg-slate-50 border border-slate-200 rounded px-1 focus:border-indigo-500 outline-none"
                                                  value={q.marks}
                                  ...
                                  <button onClick={() => removeFromPaper(q.id)} className="absolute -left-8 top-0 p-1.5 bg-white border border-red-100 text-red-500 rounded shadow-sm opacity-0 group-hover:opacity-100 transition-opacity">
      ...
      {/* === 1. SINGLE QUESTION PREVIEW POPUP === */}
      {previewQuestion && (
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/80 backdrop-blur-sm p-4">
              <div className="bg-white w-full max-w-4xl h-[90vh] rounded-2xl shadow-2xl flex flex-col overflow-hidden">
                  <div className="p-4 border-b border-slate-200 flex justify-between items-center bg-slate-50">
                      <div>
                          <h3 className="font-black text-slate-800 text-lg">Exam Overview</h3>
                          <p className="text-xs text-slate-500">Read-only preview. Click 'Save & Exit' to finalize.</p>
                      </div>
                      <button onClick={() => setShowPaperOverview(false)} className="p-2 bg-white border border-slate-200 rounded-lg hover:bg-slate-100 transition-colors">
                          <X className="w-5 h-5"/>
                      </button>
                  </div>
                  ...
                  <div className="flex-1 overflow-y-auto p-8 bg-slate-100 flex justify-center">
                      <div className="bg-white shadow-lg w-[210mm] min-h-[297mm] p-[20mm] pointer-events-none scale-90 origin-top">
                          {/* Re-rendering the paper content for read-only view */}
                          ...
                          {showInstructions && <div className="mb-8 text-sm text-slate-800 leading-relaxed" dangerouslySetInnerHTML={{__html: instructions}}></div>}
                          <div className="space-y-6">
                              {selectedQuestions.map((q, idx) => (
                                  <div key={idx} className="flex gap-3 items-start">
                                      <span className="font-bold text-lg w-6 flex-shrink-0 text-slate-900">{idx + 1}.</span>
                                      <div className="flex-1">
                                          <div className="font-medium text-slate-900 text-sm mb-2" dangerouslySetInnerHTML={{__html: q.question_text}}></div>
                                          {q.question_type === 'mcq' && q.options && (
