    ...
    <div className="min-h-screen bg-slate-950 pt-24 pb-20">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center max-w-3xl mx-auto pt-10 mb-16">
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-indigo-500/10 border border-indigo-500/20 text-indigo-400 font-bold text-[10px] mb-6">
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          <aside className="hidden lg:block w-[280px] shrink-0 sticky top-30 h-fit space-y-8 animate-in fade-in slide-in-from-left-4 duration-500">
            ...
            <div className="bg-gradient-to-br from-indigo-600 to-violet-700 rounded-[2rem] p-6 text-white shadow-xl shadow-indigo-900/20 group overflow-hidden relative">
                ...
                <h4 className="font-bold tracking-tight text-lg leading-tight mb-2 relative z-10">Tailored for <br/>your exams</h4>
                ...
                <Link href="/about" className="mt-4 flex items-center gap-2 text-[10px] font-bold bg-white/20 hover:bg-white/30 backdrop-blur-md w-fit px-3 py-1.5 rounded-full transition-all relative z-10">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex flex-wrap items-center gap-2 mb-6">
                {selectedSegment !== 'all' && (
                    <button onClick={() => {setSelectedSegment('all'); setSelectedGroup('all');}} className="flex items-center gap-1.5 px-3 py-1.5 bg-indigo-500/10 border border-indigo-500/20 text-indigo-400 rounded-lg text-[10px] font-bold tracking-tight hover:bg-indigo-500/20 transition-all">
                        {segments.find(s => s.id.toString() === selectedSegment)?.title} <X className="w-3 h-3"/>
                    </button>
                )}
                {selectedGroup !== 'all' && (
                    <button onClick={() => setSelectedGroup('all')} className="flex items-center gap-1.5 px-3 py-1.5 bg-slate-800 border border-slate-700 text-slate-400 rounded-lg text-[10px] font-bold tracking-tight hover:text-white transition-all">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                        ...
                        >
                            ...
                            <div className="relative z-10 flex items-start justify-between mb-auto">
                                <div className="w-12 h-12 bg-slate-800 rounded-2xl flex items-center justify-center text-indigo-400 group-hover:bg-indigo-600 group-hover:text-white transition-all shadow-inner border border-slate-700">
                                 ...
                                 <div className="text-[10px] font-bold text-slate-500 group-hover:text-indigo-400 transition-colors">
                                    {sub.groups?.segments?.title || 'General'}
                                </div>
                            </div>
                            
                            <div className="relative z-10">
                                <p className="text-[10px] font-bold text-slate-500 mb-1">{sub.groups?.title || 'No Group'}</p>
                                <h3 className="text-2xl font-bold text-white group-hover:text-indigo-300 transition-colors tracking-tight font-bangla mb-4">{sub.title}</h3>
                                <div className="flex items-center gap-2 text-indigo-400 font-bold text-xs group-hover:translate-x-2 transition-transform">
                ...
                {!isLoading && filteredSubjects.length === 0 && (
                    <div className="col-span-full py-20 text-center bg-slate-900/50 rounded-[3rem] border-2 border-dashed border-slate-800">
                        ...
                        <h3 className="text-xl font-bold text-white tracking-tight">No subjects found</h3>
                        ...
                        <button onClick={() => {setSearchTerm(''); setSelectedSegment('all'); setSelectedGroup('all');}} className="mt-6 px-6 py-2 bg-indigo-600 text-white font-bold rounded-xl hover:bg-indigo-700 transition-all text-xs">Clear everything</button>
                    </div>
                )}
            </div>
          </div>
...
function SidebarContent({ segments, groups, selectedSegment, setSelectedSegment, selectedGroup, setSelectedGroup, activeGroups }: any) {
    return (
        <div className="p-6 space-y-10">
            {/* Segment Section */}
            <div>
                <h4 className="text-[10px] font-bold text-slate-500 mb-4 tracking-wide">Segment</h4>
                <div className="space-y-1">
                    <button 
                        onClick={() => { setSelectedSegment('all'); setSelectedGroup('all'); }}
                        className={`w-full text-left px-4 py-3 rounded-xl text-xs font-bold transition-all ${selectedSegment === 'all' ? 'bg-indigo-600 text-white shadow-lg' : 'text-slate-400 hover:bg-slate-800 hover:text-white'}`}
                    >
                        All segments
                    </button>
                    {segments.map((seg: any) => (
                        <button 
                            ...
                            className={`w-full text-left flex justify-between items-center px-4 py-3 rounded-xl text-xs font-bold transition-all ${selectedSegment === seg.id.toString() ? 'bg-indigo-600 text-white shadow-lg' : 'text-slate-400 hover:bg-slate-800 hover:text-white'}`}
                        >
                            {seg.title}
            ...
            <div className={`transition-all duration-500 ${selectedSegment === 'all' ? 'opacity-30 pointer-events-none grayscale' : ''}`}>
                <h4 className="text-[10px] font-bold text-slate-500 mb-4 tracking-wide">Focus Group</h4>
                <div className="space-y-1">
                    <button 
                        onClick={() => setSelectedGroup('all')}
                        className={`w-full text-left px-4 py-3 rounded-xl text-xs font-bold transition-all ${selectedGroup === 'all' ? 'bg-slate-800 text-white' : 'text-slate-400 hover:bg-slate-800 hover:text-white'}`}
                    >
                        All groups
                    </button>
                    {activeGroups.map((grp: any) => (
                        <button 
                            ...
                            className={`w-full text-left flex justify-between items-center px-4 py-3 rounded-xl text-xs font-bold transition-all ${selectedGroup === grp.id.toString() ? 'bg-slate-800 text-white' : 'text-slate-400 hover:bg-slate-800 hover:text-white'}`}
                        >
                            {grp.title}
