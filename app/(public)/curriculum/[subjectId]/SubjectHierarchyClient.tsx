    ...
    <div className="w-full">
             ...
             <div className="flex-1 overflow-y-auto space-y-4 pr-3 custom-scrollbar">
                {relatedBooks.map(book => (
                  <a key={book.id} href={book.url || '#'} target="_blank" rel="noopener noreferrer" className="flex items-center gap-5 group p-5 bg-slate-50 dark:bg-slate-800/50 rounded-[2rem] hover:bg-slate-100 dark:hover:bg-slate-800 border border-transparent hover:border-indigo-500/30 transition-all shadow-sm">
                     ...
                     <div className="flex-1 min-w-0">
                        <p className="text-sm font-black text-slate-900 dark:text-slate-100 group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors truncate uppercase tracking-tight">{book.title}</p>
                        {book.subtitle && <p className="text-[10px] text-slate-500 mt-1 uppercase tracking-widest font-black truncate">{book.subtitle}</p>}
                     </div>
                ...
                {relatedBooks.length === 0 && (
                    <div className="py-20 text-center opacity-40 grayscale flex flex-col items-center gap-4">
                        <BookOpen size={48} />
                        <p className="text-xs font-black uppercase tracking-widest text-slate-500">No assets mapped</p>
                    </div>
                )}
             </div>
           </div>
      ...
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-end gap-10 mb-16 border-b border-slate-100 dark:border-slate-800 pb-16 transition-colors">
         <div className="space-y-4">
            <div className="flex flex-wrap items-center gap-3 text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-[0.2em] mb-4">
         ...
         <div className="flex flex-col sm:flex-row items-center gap-6 w-full lg:w-auto">
            ...
            <div className="flex p-2 bg-white dark:bg-slate-900 rounded-[1.5rem] border border-slate-100 dark:border-slate-800 w-full sm:w-auto shadow-xl dark:shadow-none transition-colors">
              <button 
                 ...
                 className={`flex-1 sm:flex-none px-10 py-3.5 rounded-2xl text-[10px] font-black uppercase tracking-[0.2em] transition-all ${version === 'bn' ? 'bg-slate-900 dark:bg-indigo-600 text-white shadow-xl shadow-indigo-600/20' : 'text-slate-400 hover:text-slate-900 dark:hover:text-white'}`}
              >
                 Bengali
              </button>
              <button 
                 onClick={() => setVersion('en')}
                 className={`flex-1 sm:flex-none px-10 py-3.5 rounded-2xl text-[10px] font-black uppercase tracking-[0.2em] transition-all ${version === 'en' ? 'bg-slate-900 dark:bg-indigo-600 text-white shadow-xl shadow-indigo-600/20' : 'text-slate-400 hover:text-slate-900 dark:hover:text-white'}`}
              >
                 English
              </button>
            </div>
            ...
            <div className="flex items-center gap-8 px-10 py-4 bg-white dark:bg-slate-900 rounded-[1.5rem] border border-slate-100 dark:border-slate-800 w-full sm:w-auto shadow-xl dark:shadow-none transition-colors">
               <div className="text-center group">
                  <p className="text-[9px] font-black text-slate-400 dark:text-slate-500 flex items-center gap-2 justify-center uppercase tracking-widest mb-1 group-hover:text-indigo-500 transition-colors"><Layers className="w-3.5 h-3.5"/> Modules</p>
                  <p className="text-2xl font-black text-slate-900 dark:text-white tracking-tighter">{units.reduce((acc, curr) => acc + (curr.lesson_plan_lessons?.length || 0), 0)}</p>
               </div>
               <div className="w-px h-10 bg-slate-100 dark:bg-slate-800"></div>
               <div className="text-center group">
                  <p className="text-[9px] font-black text-slate-400 dark:text-slate-500 flex items-center gap-2 justify-center uppercase tracking-widest mb-1 group-hover:text-indigo-500 transition-colors"><Eye className="w-3.5 h-3.5"/> Readers</p>
                  <p className="text-2xl font-black text-slate-900 dark:text-white tracking-tighter">{subject?.view_count || 0}</p>
               </div>
            </div>
         </div>
      </div>
           ...
           <div className="space-y-6">
                  ...
                  <div key={unit.id} className="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-[2.5rem] overflow-hidden hover:shadow-2xl dark:hover:shadow-indigo-900/10 transition-all group shadow-sm">
                     ...
                     >
                       <div className="flex items-center gap-6 sm:gap-10">
                          ...
                          <div>
                             <p className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-1 group-hover:text-indigo-500 transition-colors">Unit {unit.order_index || 'X'}</p>
                             <h3 className="text-xl sm:text-2xl font-black text-slate-900 dark:text-white tracking-tighter uppercase leading-tight">{unit.title}</h3>
                          </div>
                       </div>
                              ...
                              <div key={lesson.id} className="bg-slate-50 dark:bg-slate-800/30 border border-slate-100 dark:border-slate-700/50 rounded-[2rem] p-8 hover:border-indigo-500/30 transition-all relative overflow-hidden shadow-inner group/lesson">
                                 <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-8 gap-4 border-b border-slate-100 dark:border-slate-700/50 pb-6">
                                    <div className="flex items-center gap-4">
                                       ...
                                       <h4 className="text-lg md:text-xl font-black text-slate-900 dark:text-white uppercase tracking-tight">{lesson.title}</h4>
                                    </div>
                                    <span className="text-[9px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest">{contents.length} Intelligence Points</span>
                                 </div>
                                 ...
                                 <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                    {contents.map((content: any) => (
                                      ...
                                      >
                                         ...
                                         <div className="flex-1 min-w-0">
                                            <span className="text-[11px] font-black text-slate-900 dark:text-white group-hover/item:text-indigo-600 dark:group-hover/item:text-indigo-400 transition-colors uppercase tracking-tight truncate block">{content.title}</span>
                                            <span className="text-[8px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest block mt-0.5">{content.type}</span>
                                         </div>
              ...
              {units.length === 0 && !isLoading && (
                <div className="py-32 text-center bg-white dark:bg-slate-900/50 rounded-[3rem] border-2 border-dashed border-slate-100 dark:border-slate-800 shadow-inner group">
                   ...
                   <h3 className="text-3xl font-black text-slate-900 dark:text-white uppercase tracking-tighter mb-4">Planning Synchronization</h3>
        ...
        <aside className="w-full lg:w-[380px] space-y-10">
           ...
           <div className="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-[2.5rem] p-8 shadow-xl dark:shadow-indigo-900/5 transition-colors">
              <div className="flex justify-between items-center mb-8 border-b border-slate-50 dark:border-slate-800 pb-6">
                <h3 className="text-[10px] font-black text-slate-900 dark:text-white flex items-center gap-3 uppercase tracking-widest">
                   <Book className="w-5 h-5 text-indigo-500" /> Reference Assets
                </h3>
                {relatedBooks.length > 3 && (
                  <button onClick={() => setIsAllBooksOpen(true)} className="text-[9px] font-black text-indigo-600 dark:text-indigo-400 hover:text-indigo-700 transition-colors bg-indigo-50 dark:bg-indigo-900/20 px-4 py-2 rounded-xl">View Index</button>
                )}
              </div>
              <div className="space-y-4">
                 {relatedBooks.slice(0, 3).map(book => (
                   <a key={book.id} href={book.url || '#'} target="_blank" rel="noopener noreferrer" className="flex gap-5 group cursor-pointer p-4 rounded-2xl bg-white dark:bg-slate-900 border border-slate-50 dark:border-slate-800 hover:border-indigo-500 hover:shadow-xl dark:hover:shadow-indigo-900/10 transition-all font-bangla">
                      ...
                      <div className="py-2 flex-1 min-w-0">
                         <p className="text-sm font-black text-slate-800 dark:text-slate-200 group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors truncate uppercase tracking-tight">{book.title}</p>
                         {book.subtitle && <p className="text-[9px] text-slate-500 dark:text-slate-500 mt-2 font-black uppercase tracking-widest truncate">{book.subtitle}</p>}
                      </div>
                   </a>
                 ))}
                 {relatedBooks.length === 0 && (
                    <div className="py-12 text-center opacity-40 grayscale flex flex-col items-center gap-4">
                        ...
                        <p className="text-[9px] font-black uppercase tracking-widest text-slate-500">No assets mapped</p>
                    </div>
                 )}
              </div>
           </div>
           ...
           {linkedCourses.length > 0 && (
             <div className="bg-slate-900 dark:bg-indigo-600 rounded-[2.5rem] p-10 text-white shadow-3xl relative overflow-hidden group transition-all hover:-translate-y-2">
                ...
                <div className="relative z-10">
                   ...
                   <h3 className="text-2xl font-black text-white leading-tight mb-4 uppercase tracking-tighter">Advance Your Mastery: <br/><span className="text-indigo-400 dark:text-indigo-200">{linkedCourses[0].title}</span></h3>
                   <p className="text-slate-400 dark:text-indigo-100 text-[11px] font-black uppercase tracking-widest mb-10 leading-relaxed opacity-80">Gain industrial mastery in {subject?.title} with our world-class guided curriculum.</p>
                   <Link 
                      href={`/courses/${linkedCourses[0].slug}`}
                      className="w-full inline-flex items-center justify-center gap-3 bg-white text-slate-900 px-8 py-5 rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-slate-50 transition-all active:scale-95 shadow-2xl group"
                   >
           ...
           {otherSubjects.length > 0 && (
             <div className="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-[2.5rem] p-8 shadow-xl dark:shadow-indigo-900/5 transition-colors">
                <h3 className="text-[10px] font-black text-slate-900 dark:text-white mb-8 border-b border-slate-50 dark:border-slate-800 pb-6 uppercase tracking-widest flex items-center gap-3">
                ...
                <div className="space-y-3">
                   {otherSubjects.map(sub => (
                     ...
                     >
                        <span className="text-[11px] font-black text-slate-600 dark:text-slate-400 group-hover:text-white truncate pr-4 transition-colors uppercase tracking-tight">{sub.title}</span>
           ...
           <div className="bg-slate-900 rounded-[2.5rem] p-8 shadow-3xl relative overflow-hidden group">
              ...
              <h3 className="text-[11px] font-black text-white mb-4 flex items-center gap-3 relative z-10 uppercase tracking-widest">
                 <MessageSquare className="w-5 h-5 text-indigo-400" /> Intelligence Query
              </h3>
              <p className="text-[10px] text-slate-400 font-bold mb-8 relative z-10 leading-relaxed opacity-80 uppercase tracking-widest">Contribute to the ecosystem. Encountered an anomaly in {subject?.title} syllabus?</p>
              
              <form onSubmit={handleFeedbackSubmit} className="space-y-3 relative z-10">
                 <input 
                   value={feedbackEmail}
                   onChange={(e) => setFeedbackEmail(e.target.value)}
                   className="w-full bg-slate-800 border border-slate-700 rounded-2xl px-5 py-4 text-xs outline-none focus:border-indigo-500 transition-all text-white placeholder-slate-500 font-black uppercase tracking-widest" 
                 ...
                 <textarea 
                   ...
                   className="w-full bg-slate-800 border border-slate-700 rounded-2xl px-5 py-4 text-xs outline-none focus:border-indigo-500 transition-all min-h-[120px] text-white placeholder-slate-500 font-black uppercase tracking-widest resize-none" 
                   placeholder="Intelligence Report..."
                 ></textarea>
                 <button 
                   disabled={isSubmitting}
                   type="submit" 
                   className="w-full py-5 bg-indigo-600 text-white rounded-2xl font-black text-[10px] uppercase tracking-[0.2em] hover:bg-indigo-500 transition-all shadow-xl shadow-indigo-600/30 active:scale-95 disabled:opacity-50 flex items-center justify-center gap-3 group"
                 >
