...
export default async function SubjectPage({ params }: { params: Promise<{ segment_slug: string; group_slug: string; subject_slug: string }> }) {
  ...
  const { data: groupData } = await supabase.from("groups").select("id, title").eq("slug", group_slug).single();
  ...
  // 3. Fetch Content Types (Parallel Fetching)
      ...
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="mb-10 pb-8 border-b border-gray-200">
            <div className="flex items-center gap-2 text-xs font-bold text-gray-400 uppercase tracking-wider mb-3">
                ...
                <span className="text-gray-400">{segmentData.title}</span> / 
            ...
            <h1 className="text-4xl font-extrabold text-gray-900 mb-4">{subject.title}</h1>
            <p className="text-lg text-gray-500 max-w-3xl">
                Access complete study materials, chapter-wise notes, video lectures, and previous year board questions.
            </p>
        </div>
                    ...
                    {studyMaterials && studyMaterials.length > 0 ? (
                        <div className="grid grid-cols-1 gap-4">
                            {studyMaterials.map((item) => (
                                <div key={item.id} className="bg-white p-5 rounded-xl border border-gray-100 shadow-sm hover:shadow-md transition-all flex items-start gap-4 group">
                                    ...
                                    <div className="flex-1">
                                        <h3 className="font-bold text-gray-800 group-hover:text-blue-600 transition-colors mb-1">
                                            {item.type === 'pdf' ? <a href={item.content_url} target="_blank">{item.title}</a> : <a href={item.content_url} target="_blank">{item.title}</a>}
                                        </h3>
                                    ...
                                    <a href={item.content_url} target="_blank" className="px-4 py-2 bg-gray-50 text-gray-600 rounded-lg text-xs font-bold hover:bg-black hover:text-white transition">
                        ...
                        <div className="bg-white p-8 rounded-xl border border-gray-200 text-center text-gray-400 text-sm font-medium italic">
                            No notes uploaded yet.
                        </div>
                    )}
                </section>

                {/* 2. LATEST POSTS (BLOGS) - NEW SECTION */}
                <section>
                    <div className="flex items-center gap-3 mb-6">
                    ...
                    {blogs && blogs.length > 0 ? (
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            {blogs.map((blog) => (
                                <Link href={`/blog/${blog.id}`} key={blog.id} className="group bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden hover:shadow-lg transition-all flex flex-col h-full">
                                    ...
                                    <div className="p-5 flex-1 flex flex-col">
                                        <h3 className="font-bold text-lg text-gray-900 mb-2 leading-snug group-hover:text-purple-600 transition-colors line-clamp-2">
                                            {blog.title}
                                        </h3>
                                        <div className="mt-auto pt-4 flex items-center justify-between text-xs text-gray-400 font-medium border-t border-gray-50">
                                            ...
                                            <span className="text-purple-600 font-bold">Read Article →</span>
                                        </div>
                                    </div>
                                </Link>
                            ))}
                        ...
                        <div className="bg-white p-8 rounded-xl border border-gray-200 text-center text-gray-400 text-sm font-medium italic">
                            No blog posts available for this subject yet.
                        </div>
                    )}
                </section>
