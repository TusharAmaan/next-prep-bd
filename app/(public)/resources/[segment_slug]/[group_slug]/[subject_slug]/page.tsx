      ...
      <div className="max-w-4xl mx-auto">
        ...
        {(!resources || resources.length === 0) ? (
          ...
          <div className="grid gap-4">
            {resources.map((res) => {
              // SAFEGUARD: If content_url is missing, skip this item (Prevent Crash)
              if (!res.content_url) return null; 

              return (
                <a key={res.id} href={res.content_url} target="_blank" rel="noopener noreferrer" className="flex items-center p-4 border rounded hover:bg-gray-50 transition">
