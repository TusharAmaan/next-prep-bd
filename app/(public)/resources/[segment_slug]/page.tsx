...
import {
    ChevronRight, Clock, FolderOpen, FolderClosed,
    Calendar, Trophy, FileBarChart, ArrowRight,
    Sparkles, Layers, PenTool, FileText, PlayCircle,
    CalendarDays, BookOpen, User, Zap, Star, Smartphone, Download, Tag, HelpCircle,
    FlaskConical, PieChart, Landmark, Bell
} from "lucide-react";
...
export default async function SegmentPage({
                        ...
                        <div className="max-w-7xl mx-auto relative z-10">
                            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 md:gap-8">
                                ...
                                <Link
                                    ...
                                    className="px-6 md:px-8 py-3 md:py-4 bg-white/5 hover:bg-white/10 border border-white/10 rounded-xl md:rounded-2xl text-[9px] md:text-[10px] font-black tracking-widest transition-all backdrop-blur-xl flex items-center justify-center gap-3 group"
                                >
    ...
    const getGroupConfig = (title: string) => {
        const t = title.toLowerCase();
        if (t.includes("science")) {
            return {
                colors: "bg-blue-50 dark:bg-blue-500/10 text-blue-600 dark:text-blue-400",
                hoverBorder: "hover:border-blue-500 dark:hover:border-blue-400 hover:shadow-blue-500/5",
                gradient: "from-white to-blue-50/30 dark:from-slate-900 dark:to-blue-950/10",
                icon: <FlaskConical className="w-5 h-5 group-hover:scale-110 group-hover:rotate-12 transition-transform duration-300" />
            };
        }
        if (t.includes("business") || t.includes("commerce")) {
            return {
                colors: "bg-amber-50 dark:bg-amber-500/10 text-amber-600 dark:text-amber-450",
                hoverBorder: "hover:border-amber-500 dark:hover:border-amber-400 hover:shadow-amber-500/5",
                gradient: "from-white to-amber-50/30 dark:from-slate-900 dark:to-amber-950/10",
                icon: <PieChart className="w-5 h-5 group-hover:scale-110 group-hover:-rotate-12 transition-transform duration-300" />
            };
        }
        if (t.includes("humanities") || t.includes("arts")) {
            return {
                colors: "bg-rose-50 dark:bg-rose-500/10 text-rose-600 dark:text-rose-400",
                hoverBorder: "hover:border-rose-500 dark:hover:border-rose-400 hover:shadow-rose-500/5",
                gradient: "from-white to-rose-50/30 dark:from-slate-900 dark:to-rose-950/10",
                icon: <Landmark className="w-5 h-5 group-hover:scale-110 group-hover:rotate-6 transition-transform duration-300" />
            };
        }
        return {
            colors: "bg-purple-50 dark:bg-purple-500/10 text-purple-600 dark:text-purple-400",
            hoverBorder: "hover:border-purple-500 dark:hover:border-purple-400 hover:shadow-purple-500/5",
            gradient: "from-white to-purple-50/30 dark:from-slate-900 dark:to-purple-950/10",
            icon: <BookOpen className="w-5 h-5 group-hover:scale-110 transition-transform duration-300" />
        };
    };
    ...
    const getSegmentSubBadgeText = (title: string) => {
        const t = title.toUpperCase();
        if (t === 'SSC') return 'Bangladesh secondary certificate';
        if (t === 'HSC') return 'Bangladesh higher secondary certificate';
        return `Bangladesh ${title.toLowerCase()} certificate`;
    };
        ...
        <>
            ...
            <div className="min-h-screen bg-slate-50 dark:bg-slate-950 font-sans text-slate-900 dark:text-white pt-16 transition-colors duration-300 relative overflow-hidden">
                ...
                <div className="w-full grid grid-cols-1 lg:grid-cols-[280px_1fr] xl:grid-cols-[280px_1fr_340px]">
                    ...
                    {/* LEFT SIDEBAR - ACCORDION DIRECTORY */}
                    ...
                    <main className="min-w-0 p-4 md:p-6 pb-20">
                        ...
                        <section className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-4 md:p-8 relative overflow-hidden shadow-sm">
                            ...
                            <nav className="flex items-center gap-2 text-[10px] font-bold text-slate-400 dark:text-slate-500 tracking-wider mb-3 md:mb-5">
                                ...
                                <Link href={`/resources/${segment_slug}`} className="hover:text-indigo-650 transition-colors">Resources</Link>
                                <ChevronRight className="h-3 w-3" />
                                <span className="text-slate-655 dark:text-slate-300 font-semibold">{segmentData.title}</span>
                            </nav>

                            <div className="flex items-start gap-4 md:gap-6 mt-2">
                                {/* Left: Icon box */}
                                <div className="w-12 h-12 md:w-16 md:h-16 rounded-2xl bg-emerald-700 dark:bg-emerald-800 text-white flex items-center justify-center shrink-0 shadow-md">
                                    <div className="w-6 h-6 md:w-8 md:h-8 rounded-full bg-white dark:bg-slate-100 flex items-center justify-center text-emerald-700 dark:text-emerald-800">
                                ...
                                <div className="flex-1 min-w-0">
                                    {/* Sub-badge */}
                                    <div className="flex items-center gap-1.5 text-[9px] md:text-[10px] font-extrabold text-emerald-700 dark:text-emerald-450 tracking-wider mb-1">
                                        <Star className="h-3.5 w-3.5 fill-emerald-700 text-emerald-700 dark:fill-emerald-450 dark:text-emerald-450" />
                                        <span>{getSegmentSubBadgeText(segmentData.title)}</span>
                                    </div>

                                    {/* Title */}
                                    <h1 className="text-xl md:text-3xl font-extrabold text-slate-900 dark:text-white tracking-tight leading-tight">
                                        {segmentData.title} <span className="italic font-normal text-emerald-750 dark:text-emerald-450">Preparation</span> Hub
                                    </h1>

                                    {/* Description */}
                                    <p className="text-[11px] md:text-sm text-slate-500 dark:text-slate-400 mt-2 max-w-2xl leading-relaxed">
                                        Centralized resources, real-time updates, and verified study materials — tailored for {segmentData.title} success across all boards and subject groups.
                                    </p>
                                    ...
                                    <div className="flex flex-wrap items-center gap-x-8 gap-y-3 mt-6 pt-6 border-t border-slate-100 dark:border-slate-800/80">
                                        <div className="text-left">
                                            <div className="text-xl md:text-2xl font-extrabold text-emerald-750 dark:text-emerald-450">
                                                <AnimatedCounter value={groups?.length || 0} />
                                            </div>
                                            <div className="text-[10px] md:text-xs text-slate-500 dark:text-slate-400 mt-0.5">Subject groups</div>
                                        </div>
                                        <div className="text-left">
                                            <div className="text-xl md:text-2xl font-extrabold text-emerald-750 dark:text-emerald-450">
                                                <AnimatedCounter value={questionsCount || 0} />
                                            </div>
                                            <div className="text-[10px] md:text-xs text-slate-500 dark:text-slate-400 mt-0.5">Board questions</div>
                                        </div>
                                        <div className="text-left">
                                            <div className="text-xl md:text-2xl font-extrabold text-emerald-750 dark:text-emerald-450">
                                                <AnimatedCounter value={blogsCount || 0} />
                                            </div>
                                            <div className="text-[10px] md:text-xs text-slate-500 dark:text-slate-400 mt-0.5">Insights & blogs</div>
                                        </div>
                                        <div className="text-left">
                                            <div className="text-xl md:text-2xl font-extrabold text-emerald-750 dark:text-emerald-450">2027</div>
                                            <div className="text-[10px] md:text-xs text-slate-500 dark:text-slate-400 mt-0.5">Batch supported</div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div className="mt-6 md:mt-8 space-y-6 md:space-y-10">
                            ...
                            {groups && groups.length > 0 && (
                                <section>
                                    <div className="flex items-center gap-3 mb-6">
                                        ...
                                        <div>
                                            <h2 className="text-sm font-bold text-slate-900 dark:text-white tracking-tight">Academic Segments</h2>
                                            <p className="text-[11px] text-slate-400 dark:text-slate-500">Choose your subject group to access the full library</p>
                                        </div>
                                    </div>
                                    <div className="grid grid-cols-[repeat(auto-fill,minmax(220px,1fr))] gap-4 mb-8">
                                        {groups.map((group: any) => {
                                            const config = getGroupConfig(group.title);
                                            return (
                                                <Link
                                                    key={group.id}
                                                    href={`/resources/${segment_slug}/${group.slug}`}
                                                    className={`group bg-gradient-to-br ${config.gradient} p-[18px] rounded-2xl border border-slate-200/80 dark:border-slate-800/80 ${config.hoverBorder} hover:-translate-y-1.5 hover:shadow-lg transition-all duration-300 flex flex-col justify-between min-h-[160px] relative overflow-hidden`}
                                                >
                                                    ...
                                                    <div className="flex flex-col gap-3 relative z-10">
                                                        <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 ${config.colors} group-hover:scale-105 transition-transform duration-300 shadow-sm`}>
                                                            {config.icon}
                                                        </div>
                                                        <div className="pr-6">
                                                            <h3 className="text-sm font-bold text-slate-900 dark:text-white leading-tight group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors">{group.title}</h3>
                                                    ...
                                                    <div className="w-[26px] h-[26px] rounded-full bg-slate-100 dark:bg-slate-800 text-slate-400 group-hover:bg-indigo-600 group-hover:text-white flex items-center justify-center transition-all text-[11px] self-end mt-2 ml-auto relative z-10 group-hover:translate-x-0.5 duration-300">
                            ...
                            <QuestionBankSection 
                                questions={questions || []} 
                                segmentSlug={segment_slug} 
                                browseAllHref={`/resources/${segment_slug}?type=question`} 
                            />
                            ...
                            <section>
                                <div className="flex items-center justify-between mb-6 gap-4">
                                    <div className="flex items-center gap-3">
                                        <div className="p-2 bg-purple-50 dark:bg-purple-950/40 text-purple-600 dark:purple-400 rounded-lg">
                                            <Layers className="w-5 h-5" />
                                        </div>
                                        <div>
                                            <h2 className="text-sm font-bold text-slate-900 dark:text-white tracking-tight">Study Vault</h2>
                                            <p className="text-[11px] text-slate-400 dark:text-slate-500">PDFs, notes, and downloadable classes</p>
                                        </div>
                                    </div>
                                    <Link
                                        href={`/resources/${segment_slug}?type=pdf`}
                                ...
                                {materials && materials.length > 0 ? (
                                    <div className="space-y-3">
                                        {materials.map((item: any) => (
                                            <div
                                                key={item.id}
                                                className="group bg-white dark:bg-slate-900 p-4 rounded-xl border border-slate-200 dark:border-slate-800/80 hover:border-indigo-500 hover:shadow-sm transition-all flex items-center gap-4"
                                            >
                                                <div className={`w-12 h-12 rounded-xl flex items-center justify-center text-lg shrink-0 ${item.type === 'pdf' ? 'bg-rose-50 dark:bg-rose-950/20 text-rose-500' : 'bg-blue-50 dark:bg-blue-950/20 text-blue-550'}`}>
                                                    {item.type === 'pdf' ? <FileText className="w-5.5 h-5.5" /> : <PlayCircle className="w-5.5 h-5.5" />}
                                                </div>
                                                <div className="flex-1 min-w-0">
                                                    <h3 className="font-bold text-slate-800 dark:text-slate-200 text-xs md:text-sm group-hover:text-indigo-650 dark:group-hover:text-indigo-455 transition-colors truncate mb-1">
                                                        <Link href={`/material/${item.slug || item.id}`}>{item.title}</Link>
                                                    </h3>
                                                    <div className="flex items-center gap-2">
                                                        <span className="text-[9px] font-bold px-2 py-0.5 bg-slate-100 dark:bg-slate-800 text-slate-550 dark:text-slate-400 rounded tracking-wider">
                                                ...
                                                <Link
                                                    ...
                                                    className="px-4 py-2 bg-slate-900 dark:bg-white text-white dark:text-slate-900 rounded-lg text-[10px] font-bold tracking-wider hover:scale-102 transition-transform shrink-0"
                                                >
                                    ...
                                    <div className="p-12 text-center bg-white dark:bg-slate-900/50 rounded-2xl border border-dashed border-slate-250 dark:border-slate-800 text-slate-400">
                                        ...
                                        <p className="text-xs font-semibold">Material library is empty</p>
                            ...
                            <section>
                                <div className="flex items-center justify-between mb-6 gap-4">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div>
                                            <h2 className="text-sm font-bold text-slate-900 dark:text-white tracking-tight">Expert Insights</h2>
                                            <p className="text-[11px] text-slate-400 dark:text-slate-500">Mentorship, preparation guides, and board strategies</p>
                                        </div>
                                    </div>
                                    <Link
                                ...
                                {blogs && blogs.length > 0 ? (
                                    <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
                                        {blogs.map((blog: any) => (
                                            <Link
                                                ...
                                                className="group bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 p-4 hover:border-indigo-500 hover:shadow-md hover:-translate-y-0.5 transition-all duration-300 flex flex-col justify-between h-full"
                                            >
                                                <div>
                                                    ...
                                                    <div className="mb-4">
                                                        <h3 className="font-bold text-slate-850 dark:text-slate-100 text-xs md:text-sm leading-snug tracking-tight group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors line-clamp-2">
                                                            {blog.title}
                                                        </h3>
                                                    </div>
                                                </div>
                                    ...
                                    <div className="bg-slate-50 dark:bg-slate-900/50 p-12 rounded-2xl border-2 border-dashed border-slate-200 dark:border-slate-800 text-center text-slate-400">
                                        ...
                                        <p className="text-xs font-semibold">Academic journal pending articles</p>
                                    </div>
                                )}
                            </section>
