...
async function getData(slug: string) {
  // 1. Get the Segment (e.g., HSC)
  const { data: segment } = await supabase.from("segments").select("*").eq("slug", slug).single();
  
  if (!segment) return null;

  // 2. Get the Groups (Science, Business) linked to this segment
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-12 px-6">
      <div className="max-w-4xl mx-auto">
        ...
        <p className="text-gray-600 mb-8">Select your group/department.</p>

        {groups.length === 0 ? (
          <div className="p-4 bg-yellow-100 text-yellow-800 rounded border border-yellow-200">
            No groups found. (Please go to the Admin Panel and add groups like 'Science' or 'Commerce' to {segment.title})
          </div>
        ) : (
