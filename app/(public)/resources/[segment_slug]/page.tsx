...
import { ChevronRight, FileText, FolderOpen, Clock, LayoutGrid } from "lucide-react";
...
export default async function SegmentPage({ 
  ...
  // 2. Fetch ALL Segments (For Switcher)
  ...
  // =========================================================
  //  A. LIST VIEW MODE (Matches your attached image style)
  // =========================================================
  if (type) {
      return (
        <div className="min-h-screen bg-[#F8FAFC] font-sans text-slate-900">
            {/* Header */}
            <div className="bg-[#0f172a] text-white pt-28 pb-16 px-6 relative overflow-hidden">
                <div className="absolute top-[-20%] right-[-10%] w-[400px] h-[400px] bg-blue-600/20 blur-[100px] rounded-full pointer-events-none"></div>
                    ...
                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                        <div>
                            ...
                            <h1 className="text-3xl md:text-5xl font-black tracking-tight flex items-center gap-4">
                                ...
                                <span>{segmentData.title} <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-cyan-300">{getPageTitle()}</span></span>
                            </h1>
                        </div>
                        <Link 
            ...
            <div className="max-w-7xl mx-auto px-4 md:px-8 py-10 grid grid-cols-1 lg:grid-cols-12 gap-8">
                ...
                <div className="lg:col-span-8">
                    {/* MaterialList component should handle the cards shown in your image */}
  ...
  // 1. SMART CATEGORY FETCHING
  // Fetch all categories for questions in this segment to build the "Browse" grid
  ...
  // Filter unique, non-null categories
  ...
  const getQuestionTag = (q: any) => {
    return q.category || q.subjects?.title || "General";
  };
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8">
                ...
                <div className="hidden md:flex items-center gap-2 bg-white/5 p-1 rounded-lg border border-white/10 backdrop-blur-sm">
                    {allSegments?.slice(0, 5).map(s => (
                        <Link key={s.id} href={`/resources/${s.slug}`} className={`px-3 py-1.5 rounded-md text-xs font-bold transition-all ${s.slug === segment_slug ? 'bg-blue-600 text-white shadow-lg' : 'text-slate-400 hover:text-white hover:bg-white/10'}`}>
                            {s.title}
                        </Link>
                    ))}
                </div>
      ...
      <section className="max-w-7xl mx-auto px-4 md:px-8 py-12">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
            ...
            <div className="lg:col-span-8 space-y-16">
                            ...
                            {groups.map((group, index) => (
                                <Link key={group.id} href={`/resources/${segment_slug}/${group.slug}`} className="group relative bg-white p-1 rounded-2xl shadow-sm hover:shadow-xl transition-all duration-300 hover:-translate-y-1">
                                    ...
                                    <div className="bg-white rounded-xl p-6 h-full flex items-center gap-5 border border-slate-100 group-hover:border-blue-100 relative z-10">
                                        ...
                                        <div>
                                            <h3 className="text-lg font-bold text-slate-900 group-hover:text-blue-600 transition-colors">{group.title}</h3>
                ...
                {/* 3. PREVIOUS QUESTIONS (REFINED) */}
                <section id="question-bank" className="scroll-mt-24">
                    <div className="flex items-center gap-3 mb-6">
                        <div className="w-10 h-10 rounded-xl bg-red-50 text-red-600 flex items-center justify-center text-xl font-bold">?</div>
                        <div>
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-8">
                            {availableCategories.map((cat: any) => (
                                <Link 
                                    ...
                                    className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm hover:border-red-300 hover:shadow-md transition-all text-center group"
                                >
                                    <FolderOpen className="w-6 h-6 text-red-400 mx-auto mb-2 group-hover:text-red-600 transition-colors" />
                                    <h4 className="text-xs font-bold text-slate-700 group-hover:text-slate-900 line-clamp-1">{cat}</h4>
                                </Link>
                            ))}
                        </div>
                    )}
                    ...
                    <div className="bg-slate-50 rounded-2xl p-1 border border-slate-100">
                        {questions && questions.length > 0 ? (
                            <div className="flex flex-col">
                                {questions.map((q) => (
                                    <Link href={`/question/${q.slug || q.id}`} key={q.id} className="flex items-center gap-4 p-4 hover:bg-white hover:shadow-sm rounded-xl transition-all group border border-transparent hover:border-slate-100">
                                        <div className="w-10 h-10 rounded-lg bg-white border border-slate-200 flex items-center justify-center text-slate-400 group-hover:text-red-500 group-hover:border-red-100 font-bold shrink-0">
                                            Q
                                        </div>
                                        <div className="flex-1 min-w-0">
                                            <h3 className="font-bold text-slate-800 text-sm md:text-base truncate group-hover:text-blue-600 transition-colors">{q.title}</h3>
                                            <div className="flex items-center gap-2 mt-1">
                                                <span className="text-[10px] font-bold text-white bg-slate-400 px-2 py-0.5 rounded-full">{getQuestionTag(q)}</span>
                                ...
                                <Link href={`/resources/${segment_slug}?type=question`} className="text-center py-3 text-xs font-bold text-slate-500 hover:text-slate-800 hover:bg-slate-200/50 rounded-b-xl transition-colors">
                                    View All Questions →
                                </Link>
                            </div>
                        ) : (
                            ...
                            {materials.map((item) => (
                                <div key={item.id} className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm hover:border-blue-300 hover:shadow-md transition-all flex items-center gap-4 group">
                                    ...
                                    <div className="flex-1 min-w-0">
                                        ...
                                        <div className="flex items-center gap-3 text-xs text-slate-400 font-bold"><span className="uppercase bg-slate-100 px-2 py-0.5 rounded text-slate-500">{Array.isArray(item.subjects) ? item.subjects[0]?.title : 'General'}</span></div>
                                    </div>
