    ...
    <div className="min-h-screen bg-white font-sans">
      ...
      <div className="bg-gray-50 py-20">
        <div className="max-w-7xl mx-auto px-6">
            <div className="text-center mb-12"><h2 className="text-3xl md:text-4xl font-extrabold text-gray-900 mb-4">Latest Educational Updates</h2><p className="text-gray-500">Stay updated with the latest notices, suggestions, and guides.</p></div>
                    ...
                    <div className="bg-white rounded-2xl p-6 md:p-8 shadow-sm border border-gray-200 min-h-[500px]">
                        {loading ? <div className="animate-pulse space-y-4"><div className="h-64 bg-gray-200 rounded-xl"></div></div> : filteredBlogs.length > 0 ? (
                            <div className="flex flex-col gap-8">
                                <div>
                                    <div className="relative group overflow-hidden rounded-2xl bg-gray-100 mb-5 aspect-video">{featuredBlog.content_url ? <img src={featuredBlog.content_url} alt={featuredBlog.title} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-105" /> : <div className="w-full h-full flex items-center justify-center bg-blue-50 text-blue-200 font-bold">No Image</div>}<div className="absolute top-4 left-4 bg-red-600 text-white text-xs font-bold px-3 py-1 rounded-full shadow-lg">Latest</div></div>
                                    <h3 className="text-2xl font-bold text-gray-900 mb-3 group-hover:text-blue-600 transition-colors"><Link href={`/blog/${featuredBlog.id}`}>{featuredBlog.title}</Link></h3>
                                ...
                                <div className="space-y-4 border-t pt-8">
                                    {listBlogs.map((blog) => (
                                        <Link href={`/blog/${blog.id}`} key={blog.id} className="flex gap-4 group items-center p-3 hover:bg-gray-50 rounded-xl transition-colors border border-transparent hover:border-gray-100">
                                            <div className="w-20 h-14 bg-gray-200 rounded-lg overflow-hidden flex-shrink-0 relative">{blog.content_url && <img src={blog.content_url} className="w-full h-full object-cover" />}</div>
                                            <div><h4 className="font-bold text-gray-800 text-sm leading-snug group-hover:text-blue-600 line-clamp-2">{blog.title}</h4><span className="text-xs text-gray-400">{new Date(blog.created_at).toLocaleDateString()}</span></div>
                                        </Link>
                                    ))}
                                </div>
                            </div>
                        ) : <div className="text-center py-20 text-gray-400"><p className="text-xl font-bold mb-2">No updates yet</p></div>}
                    </div>
                </div>
