        ...
        <div className={`flex min-h-screen transition-colors duration-300 font-sans ${isDark ? 'bg-[#0f111a]' : 'bg-[#f5f7fa]'}`}>
            ...
            <aside className={`fixed top-0 bottom-0 left-0 ${isDark ? 'bg-[#1a1d2d] border-slate-800' : 'bg-white border-slate-200'} z-50 transition-all duration-300 flex flex-col ${isSidebarCollapsed ? 'w-20' : 'w-[260px]'} ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}`}>
                ...
                <div className="h-20 flex items-center px-6 border-b border-slate-100">
                    {!isSidebarCollapsed && (
                      <div className="flex items-center gap-3">
                         ...
                         <h2 className={`text-xl font-bold tracking-tighter ${isDark ? 'text-white' : 'text-slate-900'}`}>Phoenix<span className="text-indigo-600">Admin</span></h2>
                      </div>
                    )}
                    {isSidebarCollapsed && <div className="mx-auto w-8 h-8 bg-indigo-600 rounded-lg flex items-center justify-center text-white font-bold">N</div>}
                </div>
                    ...
                    {navItems.map((item, idx) => {
                      if (item.isDivider) {
                        return !isSidebarCollapsed ? (
                          <div key={idx} className="px-4 py-4 text-[11px] font-bold tracking-widest text-slate-400">{item.label}</div>
                      ...
                      return (
                        ...
                        >
                            ...
                            {item.badge ? (
                              <span className="absolute right-3 top-1/2 -translate-y-1/2 min-w-[20px] h-5 px-1 bg-rose-500 text-white text-[11px] font-bold rounded-full flex items-center justify-center border-2 border-white">
                                {item.badge}
                              </span>
                            ) : null}
                            ...
                            {isSidebarCollapsed && (
                              <div className="absolute left-full ml-4 px-3 py-1 bg-slate-900 text-white text-[11px] font-bold rounded opacity-0 group-hover:opacity-100 pointer-events-none transition-opacity whitespace-nowrap z-[100]">
                                {item.label}
                              </div>
                            )}
                        </button>
                ...
                <div className={`p-4 border-t ${isDark ? 'border-slate-800' : 'border-slate-100'}`}>
                   ...
                   >
                     {isSidebarCollapsed ? <Menu className="w-5 h-5"/> : <div className="text-[11px] font-bold tracking-widest flex items-center gap-2">Collapse Sidebar <ChevronRight className="w-3.5 h-3.5 rotate-180"/></div>}
                   </button>
                </div>
            </aside>
                        ...
                        <div className="space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-500">
                             ...
                             <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                                <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-100 flex items-center gap-4 group hover:shadow-xl hover:-translate-y-1 transition-all">
                                   ...
                                   <div>
                                      <p className="text-[11px] font-bold tracking-widest text-slate-400 mb-1">Total Students</p>
                                      <h4 className="text-2xl font-bold text-slate-900">{stats.users.total.toLocaleString()}</h4>
                                      <p className="text-[11px] font-bold text-green-500 mt-1 flex items-center gap-1"><TrendingUp className="w-3 h-3"/> +{stats.users.trend} this month</p>
                                   </div>
                                </div>
                                <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-100 flex items-center gap-4 group hover:shadow-xl hover:-translate-y-1 transition-all">
                                   ...
                                   <div>
                                      <p className="text-[11px] font-bold tracking-widest text-slate-400 mb-1">Total Donations</p>
                                      <h4 className="text-2xl font-bold text-slate-900">৳{stats.donations.total.toLocaleString()}</h4>
                                      <p className="text-[11px] font-bold text-emerald-600 mt-1">{stats.donations.count} contributions</p>
                                   </div>
                                </div>
                                <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-100 flex items-center gap-4 group hover:shadow-xl hover:-translate-y-1 transition-all">
                                   ...
                                   <div>
                                      <p className="text-[11px] font-bold tracking-widest text-slate-400 mb-1">Study Resources</p>
                                      <h4 className="text-2xl font-bold text-slate-900">{stats.materials.total}</h4>
                                      <p className="text-[11px] font-bold text-slate-400 mt-1">Live across all segments</p>
                                   </div>
                                </div>
                                <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-100 flex items-center gap-4 group hover:shadow-xl hover:-translate-y-1 transition-all">
                                   ...
                                   <div>
                                      <p className="text-[11px] font-bold tracking-widest text-slate-400 mb-1">Pending Review</p>
                                      <h4 className={`text-2xl font-bold ${stats.pendingCount > 0 ? 'text-rose-600' : 'text-slate-900'}`}>{stats.pendingCount}</h4>
                                      <p className="text-[11px] font-bold text-rose-400 mt-1">Needs immediate attention</p>
                                   </div>
                                </div>
                             </div>
                             ...
                             <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                                <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-100 flex items-center gap-4 group hover:shadow-lg transition-all">
                                   ...
                                   <div>
                                      <p className="text-[11px] font-bold tracking-widest text-slate-400 mb-1">Total Categories</p>
                                ...
                                <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-100 flex items-center gap-4 group hover:shadow-lg transition-all">
                                   ...
                                   <div>
                                      <p className="text-[11px] font-bold tracking-widest text-slate-400 mb-1">DB Tables</p>
                                      <h4 className="text-xl font-bold text-slate-900">42</h4>
                                ...
                                <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-100 flex items-center gap-4 group hover:shadow-lg transition-all">
                                   ...
                                   <div>
                                      <p className="text-[11px] font-bold tracking-widest text-slate-400 mb-1">Active Sessions</p>
                                      <h4 className="text-xl font-bold text-slate-900">128</h4>
                                      <p className="text-[9px] font-bold text-slate-400 tracking-widest">Live Now</p>
                                   </div>
                                </div>
                                <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-100 flex items-center gap-4 group hover:shadow-lg transition-all">
                                   ...
                                   <div>
                                      <p className="text-[11px] font-bold tracking-widest text-slate-400 mb-1">System Health</p>
                                      <h4 className="text-xl font-bold text-green-500 tracking-tighter">100%</h4>
                                      <p className="text-[9px] font-bold text-slate-400 tracking-widest">Stable</p>
                                   </div>
                                </div>
                             </div>
                             ...
                             <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                                <div className="lg:col-span-2 space-y-8">
                                   <div className="bg-white rounded-[2rem] p-8 shadow-sm border border-slate-100">
                                      <div className="flex items-center justify-between mb-8">
                                         <div>
                                            <h4 className="text-xl font-bold text-slate-900">Platform Growth</h4>
                                            <p className="text-[11px] font-bold text-slate-400 tracking-widest mt-1">User activity over time</p>
                                         </div>
                                         <div className="flex gap-2">
                                            {['Month', 'Year'].map(t => <button key={t} className="px-4 py-1.5 rounded-lg text-xs font-bold tracking-widest border border-slate-100 hover:bg-slate-50">{t}</button>)}
                                         </div>
                                      </div>
                                ...
                                <div className="lg:col-span-1">
                                   ...
                                   <div className="mt-8 bg-indigo-600 rounded-[2.5rem] p-8 text-white relative overflow-hidden shadow-2xl">
                                      ...
                                      <h4 className="text-2xl font-bold mb-4 leading-tight">Manage Donations</h4>
                                      <p className="text-indigo-100 text-sm mb-6 leading-relaxed">Review and approve new contributions from the donor portal.</p>
                                      <button onClick={() => setActiveTab('donations')} className="px-6 py-3 bg-white text-indigo-700 rounded-xl text-xs font-bold tracking-widest shadow-xl hover:bg-indigo-50 transition-all">Go to Donations</button>
                                   </div>
                                </div>
                             </div>
