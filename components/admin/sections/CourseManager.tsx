...
import CourseDashboard from "@/components/admin/sections/CourseDashboard";
...
export default function CourseManager({ darkMode = false }: { darkMode?: boolean }) {
    ...
    const handleViewDashboard = (course: any) => {
        setSelectedCourse(course);
        setView('dashboard');
    };
                ...
                <>
                    ...
                    <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                        ...
                        <button 
                            ...
                            className="group bg-indigo-600 hover:bg-indigo-700 text-white px-6 py-2.5 rounded-xl font-semibold text-sm flex items-center gap-2 transition-all shadow-sm active:scale-95"
                        >
                            ...
                            {filteredCourses.map(course => (
                                ...
                                >
                                    ...
                                    <div className="h-44 bg-slate-100 dark:bg-slate-800 relative overflow-hidden">
                                        ...
                                        <div className={`absolute top-4 left-4 px-2.5 py-1 rounded-md text-[10px] font-semibold capitalize tracking-wide shadow-sm ${course.is_published ? 'bg-emerald-100 text-emerald-700' : 'bg-amber-100 text-amber-700'}`}>
                                            {course.is_published ? 'Published' : 'Draft'}
                                        </div>
                                        <div className="absolute top-4 right-4 bg-black/60 backdrop-blur-md text-white px-2.5 py-1 rounded-md text-[10px] font-semibold capitalize tracking-wide flex items-center gap-1.5 ">
                                    ...
                                    <div className="p-6 flex-1 flex flex-col">
                                        <p className="text-[10px] font-semibold capitalize tracking-wide text-indigo-600 dark:text-indigo-400 mb-2">{course.level || 'Intermediate'}</p>
                                        <h3 className="text-lg font-bold text-slate-800 dark:text-white tracking-tight line-clamp-2 leading-tight mb-4 group-hover:text-indigo-600 transition-colors">
                                            {course.title}
                                        </h3>
