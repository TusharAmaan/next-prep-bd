...
export default function CourseManager({ darkMode = false }: { darkMode?: boolean }) {
    ...
    const handleDelete = async (id: string | number) => {
        if (!confirm("Are you sure you want to delete this course? This will remove all enrollments and content links.")) return;
        ...
        <div className="space-y-8 animate-in fade-in duration-500">
            ...
            <div className="flex flex-col md:flex-row gap-4 bg-white dark:bg-slate-900 p-4 rounded-3xl border border-slate-100 dark:border-slate-800 shadow-sm">
                ...
                <div className="flex gap-2">
                    <select className="bg-slate-50 dark:bg-slate-800 border-none rounded-xl px-4 py-3 text-sm font-bold text-slate-600 dark:text-slate-300 outline-none">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                        ...
                        >
                            ...
                            <div className="h-44 bg-slate-100 dark:bg-slate-800 relative overflow-hidden">
                                ...
                                <div className="absolute top-4 right-4 bg-black/30 backdrop-blur-md text-white px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest flex items-center gap-1.5 ">
                                    <Trophy size={12} className="text-amber-400" /> Certificate
                                </div>
                            </div>
                            ...
                            <div className="p-8 flex-1 flex flex-col">
                                ...
                                <h3 className="text-xl font-black text-slate-800 dark:text-white tracking-tight line-clamp-2 leading-tight mb-4 group-hover:text-indigo-600 transition-colors uppercase">
                                    {course.title}
                                </h3>
                    ...
                    {filteredCourses.length === 0 && (
                        <div className="col-span-full py-20 text-center bg-white dark:bg-slate-900 rounded-[3rem] border-2 border-dashed border-slate-100 dark:border-slate-800">
                            ...
                            <h3 className="text-xl font-black text-slate-800 dark:text-white uppercase tracking-tight">No courses found</h3>
                            <p className="text-slate-500 dark:text-slate-400 text-sm mt-1">Start by creating your first professional course.</p>
                        </div>
                    )}
                </div>
            )}
