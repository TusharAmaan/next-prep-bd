    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          ...
          <p className="text-slate-500 font-medium mt-1">Schedule 'Question of the Day' for students.</p>
        </div>
        <button 
          onClick={() => setIsSearchOpen(true)}
          className="bg-amber-500 text-white px-6 py-2.5 rounded-xl font-semibold text-sm shadow-sm hover:bg-amber-600 transition-all flex items-center gap-2 active:scale-95"
        >
      ...
      <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm overflow-hidden">
        {loading ? (
          <div className="p-24 flex flex-col items-center">
            ...
            <p className="font-medium text-slate-500">Retrieving master schedule...</p>
          </div>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-left">
              <thead className="bg-slate-50/50 dark:bg-slate-800/50 border-b dark:border-slate-700">
                <tr>
                  <th className="px-10 py-6 text-xs font-semibold text-slate-500 tracking-wide capitalize">Date</th>
                  <th className="px-10 py-6 text-xs font-semibold text-slate-500 tracking-wide capitalize">Question text</th>
                  <th className="px-10 py-6 text-xs font-semibold text-slate-500 tracking-wide capitalize text-right">Action</th>
                </tr>
              </thead>
                    ...
                    <tr key={item.id} className={`${isPast ? 'opacity-50 grayscale-[0.5]' : ''} hover:bg-slate-50/50 dark:hover:bg-slate-800/20 transition-all`}>
                      <td className="px-10 py-8">
                        <div className="flex items-center gap-4">
                          <div className={`w-14 h-14 rounded-xl flex flex-col items-center justify-center ${isPast ? 'bg-slate-100' : 'bg-amber-100 text-amber-700'} font-bold`}>
                            <span className="text-[10px] capitalize">{new Date(item.scheduled_date).toLocaleString('default', { month: 'short' })}</span>
                          ...
                          <div>
                            ...
                            <p className="text-xs text-slate-500 font-medium capitalize tracking-wide">{isPast ? 'History' : 'Upcoming'}</p>
                          </div>
                        </div>
                      </td>
                ...
                <div>
                  ...
                  <div className="flex gap-3">
                    ...
                    <button 
                      ...
                      className="bg-indigo-600 px-5 rounded-xl text-white font-semibold hover:bg-indigo-700 transition-all"
                    >
                      {searchLoading ? <Loader2 size={20} className="animate-spin" /> : <ChevronRight size={20} />}
                    </button>
                  </div>
                </div>
                ...
                <div className="max-h-[300px] overflow-y-auto space-y-2 custom-scrollbar">
                  ...
                  {searchResults.length === 0 && !searchLoading && searchQuery && (
                    <div className="text-center py-10">
                      <p className="text-slate-500 font-medium italic text-sm">No matching questions found.</p>
