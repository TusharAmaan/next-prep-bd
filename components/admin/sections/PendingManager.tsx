    ...
    <div className="space-y-6 animate-in fade-in">
      ...
      <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm overflow-hidden">
         <table className="w-full text-left text-sm">
            <thead className="bg-slate-50 dark:bg-slate-800/50 border-b border-slate-200 dark:border-slate-700 text-slate-500 dark:text-slate-400 dark:text-slate-500 uppercase text-xs font-bold">
               <tr>
            ...
            <tbody className="divide-y divide-slate-100">
               {loading ? (
                 <tr><td colSpan={5} className="p-12 text-center text-slate-400 dark:text-slate-500"><Loader2 className="w-6 h-6 animate-spin mx-auto mb-2"/>Loading...</td></tr>
               ) : items.length === 0 ? (
                 ...
                 items.map((item) => (
                   <tr key={item.id} className="hover:bg-slate-50 dark:bg-slate-800/50 transition-colors">
                      ...
                      <td className="px-6 py-4 text-slate-600 dark:text-slate-400 dark:text-slate-500">
                        ...
                        <div className="text-xs text-slate-400 dark:text-slate-500">{activeTab === 'courses' ? item.tutor?.email : item.author?.email}</div>
                      </td>
